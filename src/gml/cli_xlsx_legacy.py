...
def init_default_xlsx_path() -> str:
    # init 默认永远创建到 home（更符合“数据本地化”）
    p = os.environ.get("FATE_FILE")
    if p:
        return p
    return str(Path("~/.fate/GML.xlsx").expanduser())


def create_template_xlsx(path: Path, days: int = 60) -> None:
    ...
    ws_tasks.append(["Task_ID", "Category", "Cadence", "Task_Name", "Notes", "Default_Minutes"])
    # 给几条示例任务（你不想要可以删掉）
    ws_tasks.append(["B001", "BODY", "daily", "Walk 20 min", "", 20])
    ws_tasks.append(["M001", "MAIN", "daily", "LeetCode 1 problem", "", 30])
    ws_tasks.append(["H001", "HOME", "daily", "Tidy room 10 min", "", 10])
    ws_tasks.append(["E001", "EXP",  "daily", "English speaking 5 min", "", 5])

    # --- Daily_Log ---
    ws_log = wb.create_sheet("Daily_Log")
    ws_log.append(["Entry_ID", "Date", "Task_ID", "Category", "Minutes", "Quality", "Notes", "Evidence", "Timestamp"])

    # --- Chest_Queue ---
    ws_chest = wb.create_sheet("Chest_Queue")
    # 你现在 mark_chest 用到：A=Date, B=Eligible, E=Revealed, F=Revealed_TS
    ws_chest.append(["Date", "Eligible", "Chest_ID", "Reward", "Revealed", "Revealed_TS"])
    today = date.today()
    for i in range(days):
        d = (today + timedelta(days=i)).isoformat()
        ws_chest.append([d, 0, "", "", 0, ""])

    wb.save(str(path))


def cmd_init(argv: list[str]) -> None:
    ap = argparse.ArgumentParser(prog="fate init", description="Initialize a local template xlsx for Fate.")
    ap.add_argument("--file", type=str, default=init_default_xlsx_path(), help="Where to create the xlsx.")
    ap.add_argument("--days", type=int, default=60, help="How many days to pre-generate in Chest_Queue.")
    ...
    create_template_xlsx(path, days=args.days)
    print(f"✅ Created template: {path}")
    print("Tip: you can set env var FATE_FILE to point to your data file.")

def backup_before_save(xlsx_path: Path) -> None:
    ...
    backup_path = backup_dir / f"{xlsx_path.stem}_{ts}{xlsx_path.suffix}"
    shutil.copy2(xlsx_path, backup_path)

def main():
    # subcommand: fate init ...
